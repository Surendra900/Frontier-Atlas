...
function getEffectiveRunSnippets(model: ModelItem | ModelDetail): RunSnippets {
  if ((model as ModelDetail).runSnippets) {
    return (model as ModelDetail).runSnippets!;
  }
  ...
  return {
    ollama: `ollama run ${ollamaTag}`,
    vllm: `vllm serve ${hfId} --port 8000 --trust-remote-code`,
    transformers: `from transformers import AutoModelForCausalLM, AutoTokenizer\nimport torch\n\nmodel_id = "${hfId}"\ntokenizer = AutoTokenizer.from_pretrained(model_id)\nmodel = AutoModelForCausalLM.from_pretrained(\n    model_id,\n    torch_dtype=torch.bfloat16,\n    device_map="auto",\n)\n\nprompt = "Explain the key intuition of this model architecture."\ninputs = tokenizer(prompt, return_tensors="pt").to(model.device)\noutputs = model.generate(**inputs, max_new_tokens=256)\nprint(tokenizer.decode(outputs[0], skip_special_tokens=True))`,
...
interface ModelPageClientProps {
  initialModel: ModelDetail | null;
  slug: string;
}

export default function ModelPageClient({ initialModel, slug }: ModelPageClientProps) {
  const cleanId = slug ? slug.toLowerCase().trim() : "";
  const { toast } = useToast();

  const [model, setModel] = useState<ModelItem | ModelDetail | null>(() => {
    if (initialModel) return initialModel;
    return cleanId ? getCachedModelBySlug(cleanId) : null;
  });
  ...
  const effectiveSnippets = useMemo(() => {
    ...
    return getEffectiveRunSnippets(model);
  }, [model]);
  ...
  const handleCopy = (text: string, tabName: string) => {
    ...
    setCopied(true);
    toast.copy(`Copied ${tabName.toUpperCase()} command to clipboard.`);
    ...
    <div className="min-h-screen bg-[#F8F7F2] pb-24">
      ...
      <div className="w-full max-w-[1370px] mx-auto px-5 md:px-10 lg:px-16 xl:px-24 pt-6 pb-12">
        ...
        <div className="space-y-8">
                    ...
                    <section className="rounded-[10px] border border-[#E5E5E0] bg-white p-5">
                      ...
                      <dl>
                        ...
                        <SidebarRow
                          ...
                          value={
                            (model as ModelDetail).hardware!.fitsOn6GbGpu
                              ? "✅ Supported (Local Q4)"
                              : "⚠️ Exceeds 6GB (Cloud/API)"
                          }
                        />
                      </dl>
                    </section>
          ...
          {/* Interactive Run & Deploy Terminal */}
          {effectiveSnippets && (
            <section className="rounded-[12px] border border-[#2B2B2B] bg-[#141414] text-white p-6 shadow-lg">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-[#2A2A2A]">
                <div className="flex items-center gap-2.5">
                  <div className="flex items-center justify-center w-7 h-7 rounded-md bg-[#FF5A1F]/20 text-[#FF5A1F]">
                    <Terminal size={16} />
                  </div>
                  <div>
                    <h2 className="text-[15px] font-bold text-white tracking-tight m-0">
                      Run & Deploy {model.name}
                    </h2>
                    <p className="text-[12px] text-[#888888] m-0">
                      Production inference snippets and local terminal runtimes
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {(model as ModelDetail).hardware?.fitsOn6GbGpu ? (
                    <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-[#10B981] bg-[#10B981]/10 border border-[#10B981]/30 px-3 py-1 rounded-full">
                      <Check size={12} /> Runs on 6GB GPU
                    </span>
                  ) : (model as ModelDetail).hardware?.minVramQuantizedGb ? (
                    <span className="inline-flex items-center gap-1.5 text-[11px] font-medium text-[#F59E0B] bg-[#F59E0B]/10 border border-[#F59E0B]/30 px-3 py-1 rounded-full">
                      <Cpu size={12} /> Min {(model as ModelDetail).hardware!.minVramQuantizedGb}GB VRAM
                    </span>
                  ) : null}
                  ...
                  <div className="flex items-center gap-1 bg-[#1F1F1F] p-1 rounded-lg border border-[#333333]">
                    {(["ollama", "vllm", "transformers", "curl"] as const).map((tab) => (
                      <button
                        ...
                        className={`px-3 py-1.5 text-[11.5px] font-bold rounded-md transition-all cursor-pointer ${
                          snippetTab === tab
                            ? "bg-[#FF5A1F] text-white shadow-xs"
                            : "text-[#888888] hover:text-white"
                        }`}
                      >
                        {tab === "transformers" ? "Python" : tab === "curl" ? "cURL" : tab.toUpperCase()}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="relative mt-4">
                <button
                  type="button"
                  onClick={() => handleCopy(effectiveSnippets[snippetTab], snippetTab)}
                  className="absolute top-3 right-3 flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-[#222222] hover:bg-[#333333] text-[12px] font-semibold text-[#E5E5E0] transition-colors border border-[#383838] cursor-pointer shadow-xs"
                >
                  {copied ? <Check size={13} className="text-emerald-400" /> : <Copy size={13} />}
                  <span>{copied ? "Copied!" : "Copy"}</span>
                </button>
                <pre className="text-[12.5px] font-mono text-[#E5E5E0] bg-[#0A0A0A] p-4 pt-5 rounded-[8px] overflow-x-auto leading-relaxed border border-[#222222]">
                  <code>{effectiveSnippets[snippetTab]}</code>
                </pre>
              </div>
            </section>
          )}
